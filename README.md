...
- 一级页面与关键二级页面是否真的能打开、能使用，而不是测试数据占位或空白页
...
- **登录前覆盖**：首启、隐私弹窗、公开首页、一级导航、公开内容页、测试数据/占位数据排查
- **登录后覆盖**：个人中心、注销、常驻隐私协议/用户协议、上传/反馈、视频/支付/订单等关键功能

如果没有测试账号，也不应该把动态验证整个跳过；至少应完成登录前覆盖，并把“因登录受限未验证”的页面单列出来。
